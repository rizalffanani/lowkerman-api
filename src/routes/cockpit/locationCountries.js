...
const TABLE = 'location_countries';
const ID_COLUMN = 'id_country';
...
const MESSAGES = {
    ...
    DATA_UPDATED: 'Country updated',
    DATA_DELETED: 'Country deleted',
...
router.post('/', authenticateToken, authorizeRole('admin'), async (req, res) => {
    const { code, name_country, phone_code } = req.body;
    if (!code || !name_country || !phone_code) return res.status(400).json({ message: MESSAGES.REQUIRED_FIELDS });
    try {
        const exists = await db(TABLE).where({ name_country }).first();
        if (exists) return res.status(400).json({ message: MESSAGES.COUNTRY_EXISTS });
        const [id] = await db(TABLE).insert({ code, name_country, phone_code });
        res.status(201).json({ message: MESSAGES.DATA_ADDED, company: { id, code, name_country, phone_code } });
    } catch (err) { handleError(res, err); }
});
...
router.route('/:id')
    .get(authenticateToken, authorizeRole('admin'), async (req, res) => {
        try {
            ...
            if (!getData) return res.status(404).json({ message: MESSAGES.COUNTRY_NOT_FOUND });
    ...
    .put(authenticateToken, authorizeRole('admin'), async (req, res) => {
        ...
        if (!code || !name_country || !phone_code) return res.status(400).json({ message: MESSAGES.REQUIRED_FIELDS });
        try {
            const updated = await db(TABLE).where({ [ID_COLUMN]: req.params.id }).update({ code, name_country, phone_code });
            if (!updated) return res.status(404).json({ message: MESSAGES.COUNTRY_NOT_FOUND });
            res.status(200).json({ message: MESSAGES.DATA_UPDATED, company: { id: req.params.id, name_country, phone_code } });
        } catch (err) { handleError(res, err); }
    })
    .delete(authenticateToken, authorizeRole('admin'), async (req, res) => {
        try {
            const deleted = await db(TABLE).where({ [ID_COLUMN]: req.params.id }).del();
            if (!deleted) return res.status(404).json({ message: MESSAGES.COUNTRY_NOT_FOUND });
