...
const ID_COLUMN = 'product_categories.id_product_category';
...
router.get('/', authenticateToken, authorizeRole(['admin']), pagination, async (req, res) => {
    ...
    try {
        ...
        const [data, totalData] = await Promise.all([
            db(TABLE)
                .select('*')
                .where('name_product_category', 'like', `%${search}%`)
...
router.post('/', authenticateToken, authorizeRole('admin'), async (req, res) => {
    ...
    if (!name_product_category) return res.status(400).json({ message: MESSAGES.REQUIRED_FIELDS });
    try {
        const exists = await db(TABLE).where({ name_product_category }).first();
        if (exists) return res.status(400).json({ message: MESSAGES.DATA_EXISTS });
        const [id] = await db(TABLE).insert({
            slug_product_category: nameToSlug(name_product_category),
            name_product_category,
            active: '1'
        });
        res.status(201).json({ message: MESSAGES.DATA_ADDED, result: { id, name_product_category } });
    } catch (err) { handleError(res, err); }
});
...
router.route('/:id')
    ...
    .put(authenticateToken, authorizeRole('admin'), async (req, res) => {
        ...
        try {
            ...
            res.status(200).json({ message: MESSAGES.DATA_UPDATED, result: { id: req.params.id, name_product_category } });
        } catch (err) { handleError(res, err); }
    });
